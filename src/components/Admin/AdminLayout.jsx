...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-950 text-gray-100">
      {/* ── Sidebar ── */}
      <aside className="w-64 shrink-0 flex flex-col bg-gray-900 border-r border-gray-800 hidden md:flex">
        {/* Logo */}
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white text-sm">UrbanStay Admin</span>
        </div>
        <div className="flex items-center gap-2">
          {navItems.map(({ to, icon: Icon, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `p-2 rounded-lg transition-colors ${isActive ? "bg-indigo-600 text-white" : "text-gray-400 hover:text-white"}`
              }
            >
