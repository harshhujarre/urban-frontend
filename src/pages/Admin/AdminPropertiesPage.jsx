        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{pagination?.total?.toLocaleString() || 0} total listings</p>
            </div>
          </div>
          <button
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500"
            />
            {search && (
          ...
          <select
            ...
            className="px-3 py-2.5 bg-gray-900 border border-gray-700 rounded-xl text-sm text-white focus:outline-none focus:border-purple-500"
          >
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800/60">
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="py-16 text-center text-gray-500">No properties found.</td></tr>
                ) : (
                  data.data.map((p) => {
                    ...
                    return (
                      <tr key={p._id} className="hover:bg-gray-800/40 transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            {p.images?.[0] ? (
                              <img src={p.images[0]} alt={p.title} className="w-10 h-10 rounded-lg object-cover shrink-0 border border-gray-700" />
                            ) : (
                              <div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center shrink-0">
                                <Building2 className="w-4 h-4 text-gray-600" />
                              </div>
                            )}
                            <p className="font-medium text-white line-clamp-1 max-w-[180px]">{p.title}</p>
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          <p className="text-white">{p.hostId?.name || "—"}</p>
                          <p className="text-xs text-gray-500">{p.hostId?.phone}</p>
                        </td>
                        <td className="px-5 py-4 text-gray-400">{p.location?.city}</td>
                        ...
                        <td className="px-5 py-4 text-gray-400">{p.views || 0}</td>
                        <td className="px-5 py-4">
